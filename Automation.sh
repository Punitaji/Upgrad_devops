...
apache_running_status(){
        echo "validate  apache2 is running"
        value=`service --status-all | grep apache2 | wc -l`
        sleep 3
        if [ $value != 0 ]; then
                echo "apache2  in running state."
        else
...
#apache2 service is enabled. (The systemctl or service commands are used to check if the services are enabled and running. Enabling apache2 as a service ensures that it runs as soon as our machine reboots. It is a daemon process that runs in the background.)
apache_services_enabled() {
...
#No of  bookkeeping of archived files
Noof_bookkeeping() {
